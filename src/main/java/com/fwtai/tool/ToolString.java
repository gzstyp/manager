...
public final class ToolString implements Serializable {
    ...
    */
    public final static List<HashMap<String,Object>> parseArrayHashMap(final Object array){
    ...
    */
    public final static ArrayList<HashMap<String,Object>> parseListHashMap(final Object array){
...
// 中文
	...
	/**获取IP,map示例:{cname=河南省新乡市, cip=61.54.100.26, cid=410700}*/
	public static final HashMap<String,String> getLocation(){
		HashMap<String,String> map = new HashMap<String, String>(0);
		try {
			final Response response = ToolOkHttp.ajaxGet("http://pv.sohu.com/cityjson?ie=utf-8");
			String json = new String(response.body().string());
			int start = json.lastIndexOf("{\"");
		 	json = json.substring(start,json.length()-1);
		 	map = ToolString.parseJsonObject(json);
		} catch (Exception e){
		}
		return map;
	}
	
	/**获取IP,判断map里的map.get("ret").equalsIgnoreCase("1")再操作,值示例:{ret=1, country=中国, province=河南, city=新乡, district=, isp=, start=-1, end=-1, type=, desc=}*/
	public final static HashMap<String,String> getLocation(final String ip){
		HashMap<String,String> map = new HashMap<String, String>(0);
		try {
			final Response response = ToolOkHttp.ajaxGet("http://int.dpool.sina.com.cn/iplookup/iplookup.php?format=js&ip="+ip);
			String json = new String(response.body().string());
			int start = json.lastIndexOf("{\"");
		 	json = json.substring(start,json.length()-1);
		 	map = ToolString.parseJsonObject(json);
	...
	*/
	public final static String sqlInject(String sqlKey){
		...
		//去掉'|"|;|\字符
		sqlKey = sqlKey.replace("'","").replace("\"","").replace(";", "").replace( "\\", "");
		//转换成小写
